...
int main()
{
    std::printf ("\nKaradag Beat - DSP tests  (%.0f Hz, %.0f BPM, pattern = %.0f samples)\n\n",
                 kSampleRate, kBpm, patternLenSamples);

    const int patLen = (int) patternLenSamples;

    std::printf ("Envelope maths\n");
    {
        Envelope e (0.0);
        e.setPoints ({ { 0.0, 0.0 }, { 1.0, 0.5 } });

        check (std::abs (e.valueAt (0.0) - 0.0)  < 1e-9, "ramp start is 0");
        check (std::abs (e.valueAt (0.5) - 0.25) < 1e-9, "ramp middle is 0.25");
        check (std::abs (e.valueAt (0.999) - 0.4995) < 1e-3, "ramp end is ~0.5");

        Envelope s (0.0);
        s.setPoints ({ { 0.0, 0.2, 0.0, true }, { 0.5, 0.8, 0.0, true } });

        check (std::abs (s.valueAt (0.3) - 0.2) < 1e-9, "step holds its value");
        check (std::abs (s.valueAt (0.7) - 0.8) < 1e-9, "second step is right");

        Envelope w (0.0);
        w.setPoints ({ { 0.25, 1.0 }, { 0.75, 0.0 } });
        check (w.valueAt (0.0) > 0.4 && w.valueAt (0.0) < 0.6, "pattern end wraps to the start");
    }
    ...
    std::printf ("\nEnvelope shift\n");
    {
        ...
        check (worst < 1.0e-6, "shifted envelope = original moved in time",
               juce::String (cases) + " cases, largest difference " + juce::String (worst, 9));
        ...
        check (drift < 1.0e-6, "a full-circle shift restores the envelope",
               "difference " + juce::String (drift, 9));
    }

    std::printf ("\nDraw mode (painting cells)\n");
    {
        Envelope flat (0.0);
        ...
        check (std::abs (flat.valueAt (0.1)) < 1e-9
               ...
               "flat envelope untouched outside the painted cell",
        ...
        check (std::abs (ramp.valueAt (0.25) - 0.125) < 1e-9
               ...
               "ramp kept before and after the cell",
        ...
        check (ok && gate.getNumPoints() == before,
               "painted gate is right and repainting adds no points",
               juce::String (gate.getNumPoints()) + " points");
    }

    std::printf ("\nFast envelope lookup matches the old algorithm exactly\n");
    {
        ...
        check (worstPlain < 1.0e-12, "binary search matches the reference",
               juce::String ((int) cases.size()) + " envelopes, largest difference " + juce::String (worstPlain, 15));
        check (worstHinted < 1.0e-12, "hinted lookup matches the reference",
               "largest difference " + juce::String (worstHinted, 15));
    }
    ...
    std::printf ("\nBypass transparency\n");
    {
        ...
        check (worst < 1e-4, "with zero delay, output = input",
               juce::String ("largest difference ") + juce::String (worst, 8));
    }

    std::printf ("\nHalf Time (half speed -> one octave down)\n");
    {
        ...
        check (f > 180.0 && f < 220.0, "400 Hz input drops to ~200 Hz",
               juce::String ("measured ") + juce::String (f, 1) + " Hz");
    }

    std::printf ("\nQuarter Time (quarter speed -> two octaves down)\n");
    {
        ...
        check (f > 85.0 && f < 115.0, "400 Hz input drops to ~100 Hz",
               juce::String ("measured ") + juce::String (f, 1) + " Hz");
    }

    std::printf ("\nStutter / Repeat (pitch must hold)\n");
    {
        ...
        check (f > 380.0 && f < 420.0, "pitch stays at 400 Hz while repeating",
               juce::String ("measured ") + juce::String (f, 1) + " Hz");

        const float jump = maxJump (out, patLen * 2, patLen);
        check (jump < 0.35f, "no clicks at step changes",
               juce::String ("largest jump ") + juce::String (jump, 4));
    }

    std::printf ("\nFreeze (a looped slice: not DC, pitch must hold)\n");
    {
        ...
        check (f > 380.0 && f < 420.0, "pitch stays at 400 Hz while frozen",
               juce::String ("measured ") + juce::String (f, 1) + " Hz");
        check (r > 0.5, "freeze doesn't collapse to DC, audio keeps going",
               juce::String ("rms ") + juce::String (r, 4));
    }

    std::printf ("\nDoes reverse really play backwards\n");
    {
        ...
        const int stepSamples = patLen / 8;
        ...
        check (early > late + 100.0,
               "step starts high, ends low (audio runs backwards)",
               juce::String ("start ") + juce::String (early, 0)
                 + " Hz  ->  end " + juce::String (late, 0) + " Hz");
    }
    ...
    std::printf ("\nGate (volume envelope)\n");
    {
        ...
        check (openRms > 0.5,    "sound on open steps",
               juce::String ("rms ") + juce::String (openRms, 4));
        check (closedRms < 0.01, "silence on closed steps",
    ...
    std::printf ("\nSidechain pump\n");
    {
        ...
        const double atBeat  = rms (out, patLen + 8, beat / 16);
        const double atEnd   = rms (out, patLen + beat - beat / 8, beat / 16);

        check (atBeat < atEnd * 0.5, "ducks at the start of the beat, then comes back",
               juce::String ("start ") + juce::String (atBeat, 4)
                 + "  end " + juce::String (atEnd, 4));
    }
    ...
    std::printf ("\nIndependent of sample rate\n");
    {
        ...
        for (const double sr : { 44100.0, 48000.0, 88200.0, 96000.0 })
        {
            ...
            check (f > 190.0 && f < 210.0,
                   juce::String ((int) sr) + " Hz: half time is right",
                   juce::String ("measured ") + juce::String (f, 1) + " Hz");
        }
    }

    std::printf ("\nIndependent of tempo (stutter mustn't bend the pitch)\n");
    {
        const auto& pr = preset ("Repeat 1/8");
        auto t = fromPoints (pr.time, 0.0);
        auto v = fromPoints (pr.volume, 1.0);
        ...
        for (const double bpm : { 70.0, 90.0, 128.0, 174.0 })
        {
            ...
            check (f > 385.0 && f < 415.0,
                   juce::String (bpm, 0) + " BPM: pitch holds",
                   juce::String ("measured ") + juce::String (f, 1) + " Hz");
        }
    }

    std::printf ("\nIndependent of block size (sample-for-sample identical output)\n");
    {
        ...
        for (const int bs : { 32, 64, 512, 2048 })
        {
            ...
            check (worst < 1e-6,
                   juce::String (bs) + "-sample blocks give the same result",
                   juce::String ("largest difference ") + juce::String (worst, 9));
        }
    }

    std::printf ("\n3/4 time (2-bar pattern = 6 quarter notes)\n");
    {
        const auto& pr = preset ("Gate 1/8");
        auto t = fromPoints (pr.time, 0.0);
        auto v = fromPoints (pr.volume, 1.0);
        ...
        check (openRms > 0.5 && closedRms < 0.01,
               "gate steps in the right place in 3/4",
               juce::String ("open ") + juce::String (openRms, 3)
                 + "  closed " + juce::String (closedRms, 5));
    }

    std::printf ("\nSurvives tempo changes\n");
    {
        const auto& pr = preset ("Half Time");
        auto t = fromPoints (pr.time, 0.0);
        auto v = fromPoints (pr.volume, 1.0);
        ...
        check (finite && peak < 1.05f && worstJump < 0.5f,
               "no overshoot or clicks while the tempo changes",
               juce::String ("peak ") + juce::String (peak, 3)
                 + "  jump " + juce::String (worstJump, 3));
    }
    ...
    std::printf ("\nProcessing time (for information)\n");
    {
        ...
        check (realtime > 20.0, "faster than real time with 128-point envelopes",
               juce::String (juce::roundToInt (realtime)) + "x real time  (60 s of audio in "
                 + juce::String (juce::roundToInt (seconds * 1000.0)) + " ms)");
    }

    std::printf ("\nClicks on switch and mix changes\n");
    {
        ...
        for (int scenario = 0; scenario < 2; ++scenario)
        {
            ...
            check (jump < 0.2f,
                   scenario == 0 ? "VOLUME switch doesn't click" : "mix change doesn't click",
                   juce::String ("largest jump ") + juce::String (jump, 3));
        }
    }
    ...
    std::printf ("\nWaveform slices\n");
    {
        ...
        check (std::abs (loudMin - 0.5f) < 1.0e-4f && quietMax < 1.0e-6f,
               "slices with audio are full, the rest empty",
               "first quarter at least " + juce::String (loudMin, 3)
                 + ", the rest at most " + juce::String (quietMax, 5));
    }
    ...
    std::printf ("\nSMOOTH times\n");
    {
        ...
        for (const double ms : { 2.0, 20.0, 60.0 })
        {
            ...
            check (hi > 0 && lo > 0 && std::abs (measured - expected) < expected * 0.05 + 0.05,
                   ...
                   "90 % -> 10 %: " + juce::String (measured, 2) + " ms, expected "
        ...
        for (const double ms : { 4.0, 30.0 })
        {
            ...
            check (hi > 0 && lo > 0 && std::abs (measured - expected) < expected * 0.05 + 0.05,
                   ...
                   "crossfade 90 % -> 10 %: " + juce::String (measured, 2) + " ms, expected "
                     + juce::String (expected, 2) + " ms");
        }
    }

    std::printf ("\nHealth check on every factory pattern (437 Hz)\n");
    {
        for (const auto& pr : Presets::factory())
        {
            auto t = fromPoints (pr.time, 0.0);
            ...
            check (finite && peak < 1.05f && jump < 0.2f,
                   pr.name,
                   juce::String ("peak ") + juce::String (peak, 3)
                     + " @phase " + juce::String (peakPhase, 3)
                     + "  jump " + juce::String (jump, 3));
        }
    }
    ...
    std::printf ("\nSwing time warp\n");
    {
        ...
        check (worstRoundTrip < 1.0e-12, "real -> pattern -> real returns to the same place",
        ...
        check (evenFixed && std::abs (oddStart - 2.0 * 0.66 / cells) < 1.0e-12,
               "beats stay put, off-beat sixteenths move",
               "sixteenth 1 moved to cell " + juce::String (oddStart * cells, 3));

        check (Swing::patternToReal (0.3, cells, 0.5) == 0.3 && ! Swing::isActive (cells, 0.5),
               "50 % swing means straight");
    }

    std::printf ("\nSwung repeat: from the start of the source step, at normal speed\n");
    {
        const int cells = 32;
        ...
        for (int pair = 0; pair < cells / 2; ++pair)
        {
            ...
            const double first = (2 * pair) / (double) cells;
        ...
        check (worstStart < 1.0e-3, "repeat starts at the start of the source step",
               "largest offset " + juce::String (worstStart * patternLenSamples, 1) + " samples");
        check (worstSlope < 1.0e-3, "repeat plays at normal speed (pitch intact)",
               "slope error " + juce::String (worstSlope, 5));
    }
    ...
    std::printf ("\nSwung gate\n");
    {
        ...
        check (worst < 2.0, "gate edges land on the swung positions",
               "largest deviation " + juce::String (worst, 1) + " samples");
    }
    ...
    std::printf ("\nFilter lane\n");
    {
...

        {
            ...
            check (diff == 0.0f, "open filter is bit-transparent",
                   "largest difference " + juce::String (diff, 9));
        }
...

        {
            ...
            check (lowDb > -1.5 && highDb < -40.0, "low-pass keeps lows, cuts highs",
                   "50 Hz " + juce::String (lowDb, 1) + " dB, 5 kHz " + juce::String (highDb, 1) + " dB  (cutoff "
            ...
            check (hpLow < -40.0 && hpHigh > -1.5, "high-pass keeps highs, cuts lows",
                   "50 Hz " + juce::String (hpLow, 1) + " dB, 12 kHz " + juce::String (hpHigh, 1) + " dB  (cutoff "
...

        {
            ...
            check (jump < 0.06f, "stepped filter envelope doesn't click",
                   "largest jump " + juce::String (jump, 4));
        }

        {
            ...
            check (finite && peak < 4.0f, "sweep at full resonance is stable",
...

        {
            ...
            check (maxJump (out, half - 2000, 4000) < 0.06f, "no click when the lane is switched off",
                   "largest jump " + juce::String (maxJump (out, half - 2000, 4000), 4));
        }
    }
